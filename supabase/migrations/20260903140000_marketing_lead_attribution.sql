-- CRM lead attribution: lead_sources join, landing_url parse, slug eşleşme

create or replace function public.url_query_param(url text, param text)
returns text
language sql
immutable
as $$
...
-- Mevcut kayıtlarda landing_url'den utm/gclid doldur
update public.lead_sources ls
set
  utm_source = coalesce(nullif(btrim(ls.utm_source), ''), public.url_query_param(ls.landing_url, 'utm_source')),
  utm_medium = coalesce(nullif(btrim(ls.utm_medium), ''), public.url_query_param(ls.landing_url, 'utm_medium')),
  utm_campaign = coalesce(nullif(btrim(ls.utm_campaign), ''), public.url_query_param(ls.landing_url, 'utm_campaign')),
  campaign = coalesce(nullif(btrim(ls.campaign), ''), public.url_query_param(ls.landing_url, 'campaign')),
  gclid = coalesce(nullif(btrim(ls.gclid), ''), public.url_query_param(ls.landing_url, 'gclid')),
  gbraid = coalesce(nullif(btrim(ls.gbraid), ''), public.url_query_param(ls.landing_url, 'gbraid')),
  wbraid = coalesce(nullif(btrim(ls.wbraid), ''), public.url_query_param(ls.landing_url, 'wbraid')),
  fbclid = coalesce(nullif(btrim(ls.fbclid), ''), public.url_query_param(ls.landing_url, 'fbclid'))
where ls.landing_url is not null;

update public.leads l
set
  utm_source = coalesce(nullif(btrim(l.utm_source), ''), ls.utm_source),
  utm_medium = coalesce(nullif(btrim(l.utm_medium), ''), ls.utm_medium),
  utm_campaign = coalesce(nullif(btrim(l.utm_campaign), ''), ls.utm_campaign),
  campaign = coalesce(nullif(btrim(l.campaign), ''), ls.campaign),
  gclid = coalesce(nullif(btrim(l.gclid), ''), ls.gclid),
  gbraid = coalesce(nullif(btrim(l.gbraid), ''), ls.gbraid),
  wbraid = coalesce(nullif(btrim(l.wbraid), ''), ls.wbraid),
  fbclid = coalesce(nullif(btrim(l.fbclid), ''), ls.fbclid)
from public.lead_sources ls
where l.lead_ref = ls.lead_ref;
