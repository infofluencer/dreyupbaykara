-- Takvim lead'lerine attribution mirası: CPL çift sayımını engelle + mevcut blank lead backfill.

-- 1) channel=calendar lead'leri Google/Meta CRM bucket'ına girmez (miras gclid/fbclid olsa bile)
...
as $$
  ...
  classified_leads as (
    select
      ...
      l.status,
      case
        -- Takvim miras lead'i: CPL'de Google/Meta olarak sayma
...
-- 2) Mevcut boş calendar/manual lead'lere first-touch sibling attribution
with blank_leads as (
  select l.id, l.contact_id
  from public.leads l
  where lower(btrim(coalesce(l.channel, ''))) in ('calendar', 'manual')
    and nullif(btrim(coalesce(l.gclid, '')), '') is null
    and nullif(btrim(coalesce(l.gbraid, '')), '') is null
    and nullif(btrim(coalesce(l.wbraid, '')), '') is null
    and nullif(btrim(coalesce(l.fbclid, '')), '') is null
    and nullif(btrim(coalesce(l.ctwa_clid, '')), '') is null
    and nullif(btrim(coalesce(l.msclkid, '')), '') is null
    and nullif(btrim(coalesce(l.ttclid, '')), '') is null
    and nullif(btrim(coalesce(l.utm_source, '')), '') is null
    and nullif(btrim(coalesce(l.utm_medium, '')), '') is null
    and nullif(btrim(coalesce(l.utm_campaign, '')), '') is null
    and nullif(btrim(coalesce(l.campaign, '')), '') is null
),
first_touch as (
  ...
  from blank_leads b
  join public.leads s
    on s.contact_id = b.contact_id
   and s.id <> b.id
  where (
    lower(btrim(coalesce(s.channel, ''))) = 'meta_ctwa'
    or nullif(btrim(coalesce(s.gclid, '')), '') is not null
    or nullif(btrim(coalesce(s.gbraid, '')), '') is not null
    or nullif(btrim(coalesce(s.wbraid, '')), '') is not null
    or nullif(btrim(coalesce(s.fbclid, '')), '') is not null
    or nullif(btrim(coalesce(s.ctwa_clid, '')), '') is not null
    or nullif(btrim(coalesce(s.msclkid, '')), '') is not null
    or nullif(btrim(coalesce(s.ttclid, '')), '') is not null
    or nullif(btrim(coalesce(s.utm_source, '')), '') is not null
    or nullif(btrim(coalesce(s.utm_medium, '')), '') is not null
    or nullif(btrim(coalesce(s.utm_campaign, '')), '') is not null
    or nullif(btrim(coalesce(s.campaign, '')), '') is not null
  )
  order by b.id, s.created_at asc
)
update public.leads l
set
  site = case
    when nullif(btrim(coalesce(ft.src_site, '')), '') is not null
      and lower(btrim(ft.src_site)) <> 'manual'
      then btrim(ft.src_site)
    else l.site
  end,
  ...
  wbraid = ft.wbraid,
  fbclid = ft.fbclid,
  ctwa_clid = ft.ctwa_clid,
  msclkid = ft.msclkid,
  ttclid = ft.ttclid,
  updated_at = now()
from first_touch ft
where l.id = ft.target_id;
