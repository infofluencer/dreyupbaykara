-- Remove randevulu lead status; surgery-only calendar; disable appt_* WA rules.

-- 1) Migrate existing randevulu leads
update public.leads
set status = 'muayene_edildi'
where status = 'randevulu';

update public.lead_status_history
set from_status = 'muayene_edildi'
where from_status = 'randevulu';

update public.lead_status_history
set to_status = 'muayene_edildi'
where to_status = 'randevulu';

-- 2) Tighten status check (drop randevulu)
...
comment on column public.message_rules.lead_statuses is
  'Durum Panosu lead status filtreleri (yeni, arandi, muayene_edildi, ameliyat_olacak, ameliyat_edildi, bitti). Boş = filtre yok.';

-- 3) Disable consultation appointment reminders; scrub randevulu from rule filters
update public.message_rules
set
  enabled = false,
  lead_statuses = array_remove(lead_statuses, 'randevulu')
where key in ('appt_1d', 'appt_1h');

update public.message_rules
set lead_statuses = array_remove(lead_statuses, 'randevulu')
where 'randevulu' = any (lead_statuses);

-- 4) Marketing: Lead → Ameliyat (appointment_leads)
